...
TEST(MumpsAdapterTest, ExportLocalBlockSimple)
{
    ...
    values.push_back(static_cast<Scalar>(10.0));
    values.push_back(static_cast<Scalar>(20.0));
    values.push_back(static_cast<Scalar>(30.0));
    ...
    EXPECT_EQ(IRN[0], 1);
    EXPECT_EQ(JCN[0], 1);
    EXPECT_EQ(A[0], static_cast<Scalar>(10.0));
    EXPECT_EQ(IRN[1], 1);
    EXPECT_EQ(JCN[1], 3);
    EXPECT_EQ(A[1], static_cast<Scalar>(20.0));
    EXPECT_EQ(IRN[2], 2);
    EXPECT_EQ(JCN[2], 2);
    EXPECT_EQ(A[2], static_cast<Scalar>(30.0));
}
...
TEST(MumpsAdapterTest, ExportLocalBlockNonZeroOffset)
{
    ...
    vValues.push_back(static_cast<Scalar>(5.0));
    vValues.push_back(static_cast<Scalar>(7.0));
    vValues.push_back(static_cast<Scalar>(9.0));
    cCSRMatrix<Scalar> cLocal(2, 4, 6, vRowPtr, vColInd, vValues);
    cRowDistribution dist = cRowDistribution::CreateBlockDistribution(6, 3, 1); // rank=1 owns [2,4)

    std::vector<int> vIRN;
    std::vector<int> vJCN;
    std::vector<Scalar> vA;
    cMumpsAdapter<Scalar>::ExportLocalBlock(cLocal, dist, vIRN, vJCN, vA);
    ...
    EXPECT_EQ(vJCN[1], 4);
    EXPECT_EQ(vJCN[2], 3);
    EXPECT_EQ(vA[0], static_cast<Scalar>(5.0));
    EXPECT_EQ(vA[1], static_cast<Scalar>(7.0));
    EXPECT_EQ(vA[2], static_cast<Scalar>(9.0));
}
...
TEST(MumpsAdapterTest, ExportLocalBlockIntoPreallocated)
{
    ...
    std::vector<Scalar> vValues { static_cast<Scalar>(2.0), static_cast<Scalar>(4.0), static_cast<Scalar>(8.0) };
    cCSRMatrix<Scalar> cLocal(4, 6, 10, vRowPtr, vColInd, vValues); // global rows [4,6)
    ...
    std::vector<Scalar> vA(iLocalNNZ);
    iSize iWritten = cMumpsAdapter<Scalar>::ExportLocalBlockInto(cLocal, dist, vIRN.data(), vJCN.data(), vA.data());
    ...
    EXPECT_EQ(vIRN[2], 6);
    EXPECT_EQ(vJCN[2], 3);
    EXPECT_EQ(vA[0], static_cast<Scalar>(2.0));
    EXPECT_EQ(vA[1], static_cast<Scalar>(4.0));
    EXPECT_EQ(vA[2], static_cast<Scalar>(8.0));
}
...
TEST(MumpsAdapterTest, ExportLocalBlockAppendPreservesExisting)
{
    ...
    std::vector<Scalar> vValues { static_cast<Scalar>(1.0), static_cast<Scalar>(3.0) };
    cCSRMatrix<Scalar> cLocal(0, 1, 5, vRowPtr, vColInd, vValues);
    cRowDistribution dist = cRowDistribution::CreateBlockDistribution(1, 1, 0);
    std::vector<int> vIRN { -999 };
    std::vector<int> vJCN { -888 };
    std::vector<Scalar> vA { static_cast<Scalar>(-1.0) };
    cMumpsAdapter<Scalar>::ExportLocalBlock(cLocal, dist, vIRN, vJCN, vA);
    ASSERT_EQ(vIRN.size(), 1u + 2u);
    // Existing sentinel preserved
    EXPECT_EQ(vIRN[0], -999);
    EXPECT_EQ(vJCN[0], -888);
    EXPECT_EQ(vA[0], static_cast<Scalar>(-1.0));
    // Appended entries (row 0 -> 1-based 1)
    EXPECT_EQ(vIRN[1], 1);
    EXPECT_EQ(vJCN[1], 1);
    EXPECT_EQ(vA[1], static_cast<Scalar>(1.0));
    EXPECT_EQ(vIRN[2], 1);
    EXPECT_EQ(vJCN[2], 3);
    EXPECT_EQ(vA[2], static_cast<Scalar>(3.0));
}
...
TEST(MumpsAdapterTest, ExportLocalBlockEmpty)
{
    ...
    std::vector<Scalar> vValues;
    cCSRMatrix<Scalar> cLocal(0, 0, 4, vRowPtr, vColInd, vValues);
