...
static cCSRMatrix<Scalar> LoadSymLower(const std::string& rel)
{
    ...
    return cCSRMatrix<Scalar>(0, rows, cols, rp, ci, vv, eSymm);
}
...
TEST(SymmetricTest, SpMMLowerTriangleExpansion)
{
    ...
    std::vector<Scalar> X { static_cast<Scalar>(1), static_cast<Scalar>(2), static_cast<Scalar>(3),
        static_cast<Scalar>(2), static_cast<Scalar>(0), static_cast<Scalar>(1) };
    std::vector<Scalar> Y;
    SpMM(A, X, nCols, Y);
    ...
    EXPECT_EQ(Y[1], static_cast<Scalar>(13));
    EXPECT_EQ(Y[2], static_cast<Scalar>(19));
    EXPECT_EQ(Y[3], static_cast<Scalar>(8));
    EXPECT_EQ(Y[4], static_cast<Scalar>(4));
    EXPECT_EQ(Y[5], static_cast<Scalar>(5));
}
