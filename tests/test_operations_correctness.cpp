...
// Row2: 4 5 0 0 6
// Row3: 0 0 7 0 8
static cCSRMatrix<Scalar> BuildMatrix4x5()
{
    ...
    std::vector<Scalar> vValues = { 1, 2, 3, 4, 5, 6, 7, 8 };
...
TEST(TestOperationsCorrectness, SpMVMatchesDense)
{
    ...
    std::vector<Scalar> x = { 10, 11, 12, 13, 14 };
    ...
    std::vector<Scalar> dense(rows * cols, static_cast<Scalar>(0));
    // Fill from pattern
    dense[0 * cols + 0] = static_cast<Scalar>(1);
    dense[0 * cols + 2] = static_cast<Scalar>(2);
    dense[1 * cols + 3] = static_cast<Scalar>(3);
    dense[2 * cols + 0] = static_cast<Scalar>(4);
    dense[2 * cols + 1] = static_cast<Scalar>(5);
    dense[2 * cols + 4] = static_cast<Scalar>(6);
    dense[3 * cols + 2] = static_cast<Scalar>(7);
    dense[3 * cols + 4] = static_cast<Scalar>(8);
    std::vector<Scalar> y_ref(rows, static_cast<Scalar>(0));
...
TEST(TestOperationsCorrectness, SpMMMatchesDense)
{
    ...
    // col1: 6 7 8 9 10
    // col2: 11 12 13 14 15
    std::vector<Scalar> X(cols * nCols, static_cast<Scalar>(0));
    for (iSize c = 0; c < cols; ++c) {
        X[static_cast<size_t>(c + 0 * cols)] = static_cast<Scalar>(1 + c);
        X[static_cast<size_t>(c + 1 * cols)] = static_cast<Scalar>(6 + c);
        X[static_cast<size_t>(c + 2 * cols)] = static_cast<Scalar>(11 + c);
    }
    ...
    std::vector<Scalar> Y_ref(rows * nCols, static_cast<Scalar>(0));
