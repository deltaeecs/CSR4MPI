...
namespace csr4mpi {
void cCSRComm::Assemble(cCSRMatrix& cLocal,
    ...
    MPI_Comm cComm)
{
    ...
    std::vector<vScalar>& vLocalVal = const_cast<std::vector<vScalar>&>(cLocal.vValues());
    ...
    MPI_Datatype tScalarType;
    bool bCreatedScalarType = false;
    {
        // Map vScalar -> MPI_Datatype
#if CSR4MPI_VALUE_TYPE == 0
        tScalarType = MPI_FLOAT;
#elif CSR4MPI_VALUE_TYPE == 1
        tScalarType = MPI_DOUBLE;
#elif CSR4MPI_VALUE_TYPE == 2
#ifdef MPI_C_FLOAT_COMPLEX
        tScalarType = MPI_C_FLOAT_COMPLEX;
#else
        MPI_Type_contiguous(2, MPI_FLOAT, &tScalarType);
        MPI_Type_commit(&tScalarType);
        bCreatedScalarType = true;
#endif
#elif CSR4MPI_VALUE_TYPE == 3
#ifdef MPI_C_DOUBLE_COMPLEX
        tScalarType = MPI_C_DOUBLE_COMPLEX;
#else
        MPI_Type_contiguous(2, MPI_DOUBLE, &tScalarType);
        MPI_Type_commit(&tScalarType);
        bCreatedScalarType = true;
#endif
#else
#error "Unsupported CSR4MPI_VALUE_TYPE in CSRComm.cpp"
#endif
    ...
    for (const cTriplet& cT : vRecvBuf) {
        ...
        // Map global row to local row using cRowDistribution on the caller side
        // when building vValues and pattern.
        // Here we assume that global row belongs to this rank and local row is
        // simply offset from cLocal.iGlobalRowBegin().
